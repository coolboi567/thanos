...
func SyncAll(
	...
	u1, err := s.SyncCompactedOnce(ctx)
	if err != nil {
		merr.Add(err)
	}
...
// Experimental. Sync only those which are seen once? based on file and first view.
func (s *Shipper) SyncCompactedOnce(ctx context.Context) (uploaded int, err error) {
	var (
