...
func registerBucketSync(m map[string]setupFunc, root *kingpin.CmdClause, name string, objStoreConfig *pathOrContent) {
	cmd := root.Command("sync",
		`[Experimental] Synchronise all blocks with remote bucket. NOTE: Currently NO compactor must be running in the same time.`)
	...
	dataDir := cmd.Flag("data-dir", "Data to sync local blocks from. Usually directory of Prometheus TSDB. If empty, snapshoted blocks from Prometheus will be used. (recommended if Prometheus is up)").
		String()
	...
	m[name+" sync"] = func(g *run.Group, logger log.Logger, reg *prometheus.Registry, _ opentracing.Tracer, _ bool) error {
		...
		if len(lset) == 0 {
			...
			lset, err = promclient.ExternalLabels(ctx, logger, *promURL)
			if err != nil {
		...
		if len(lset) == 0 {
			return errors.Wrap(err,  "syncing blocks without external labels is not allowed. Set unique external labels in your Prometheus config or pass explicit external labels via --labels flag.")
		}

		if *dataDir == "" {
			level.Info(logger).Log("msg", "--data-dir is not specified; requesting Prometheus snapshot to sync data from", "addr", *promURL)
			f, err := promclient.ConfiguredFlags(ctx, logger, *promURL)
			if err != nil {
				return errors.Wrapf(err, "get Prometheus flags on %s", *promURL)
			}
			dir, err := promclient.Snapshot(ctx, logger, *promURL, false)
			if err != nil {
				return errors.Wrapf(err, "snapshot Prometheus on %s", *promURL)
			}

			*dataDir = path.Join(f.TSDBPath, dir)
		}
		...
		uploaded, err := shipper.SyncAll(ctx, logger, *dataDir, bkt, lsetFn, metadata.BucketSyncSource)
		if err != nil {
			if uploaded > 0 {
